...
```{r}
schools<-read.csv("directory with decile_christchurch_secondary_with_zone.csv")
```
...
```{r}
# covert date to character of year and month format
d<- format(d, "%Y-%m")
```
```{r}
class(d)
...
```{r}
schoolid_list<-sort(schoolid_list)
```
...
```
```{r}
table(df$threeyears_316)
```

```{r}
df$CL_Latitude[273430]
```
```{r}
sort(unique(df$CL_Latitude)) 
```
```{r}
...
```{r}
head(sort(unique(df$CL_Longitude)))
```
...
```{r}
write.csv(df, "chapter3_dataset_afterrunning002_data_prep.csv")
```

```{r}
write.csv(schoolid_list, "schoolid_list.csv")
```
